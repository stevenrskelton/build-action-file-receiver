...
import ca.stevenskelton.buildactionfilereceiver.FileUploadFormData.FileUploadFieldName
...
object UploadRequestHelper {
  ...
  def httpApp(
    ...
    for
      tmpDir <- Files[IO].createTempDirectory(None, "build-action-file-receiver-specs-", None)
    yield {
