...
object FileUploadDirectives {
  ...
  def parseFormData(formData: Multipart.FormData, ctx: RequestContext): Future[(GithubPackage, FileInfo, Source[ByteString, Any])] = {
    ...
    val done = Promise[(GithubPackage, FileInfo, Source[ByteString, Any])]()
