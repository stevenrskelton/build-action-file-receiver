...
import org.http4s.{Entity, Header, Headers, MalformedMessageBodyFailure}
...
object MultipartParser {
  ...
  def parseStreamed[F[_] : Concurrent](
    ...
    val parts = st.through(
      parseToPartsStream(boundary, limit, maxParts, failOnLimit)
    )
    Multipart(parts, boundary)
  }

  def parseToPartsStream[F[_]](boundary: Boundary, limit: Int = 1024, maxParts: Int = 20, failOnLimit: Boolean = false)(implicit
                                                                                                                        F: Concurrent[F]
  ): Pipe[F, Byte, Part[F]] = { st =>
    st.through(
        parseEvents[F](boundary, limit)
      )
      .through(limitParts(maxParts, failOnLimit))
  ...
  private def splitAndIgnorePrev[F[_]](
    ...
    while currState < len && i < c.size do {
      if c(i) == values(currState) then
        currState += 1
      else if c(i) == values(0) then
        currState = 1
      else
        currState = 0
      i += 1
  ...
  private def splitOrFinish[F[_] : Concurrent](
    ...
    def checkIfLast(c: Chunk[Byte], rest: Stream[F, Byte]): SplitStream[F] = {
      ...
      def checkTwoNonEmpty(
        ...
        if c1(0) == dashByte && c2(0) == dashByte then
      ...
      else if c(0) == dashByte && c(1) == dashByte then
        // Drain the multipart epilogue.
        Pull.eval(rest.compile.drain) *>
          Pull.pure((streamEmpty, streamEmpty))
      else {
   ...
   */
  private def splitPartialLimited[F[_]](
                                         state: Int,
                                         middleChunked: Boolean,
                                         currState: Int,
                                         i: Int,
  ...
  private def splitOnChunkLimited[F[_]](
                                         values: Array[Byte],
                                         state: Int,
                                         c: Chunk[Byte],
                                         acc: Stream[F, Byte],
    ...
    def go(st: Stream[F, Event], partsCounter: Int): Pull[F, Event, Unit] =
      st.pull.uncons1.flatMap {
        case Some((event: PartStart, rest)) =>
          if partsCounter < maxParts then {
            Pull.output1(event) >> go(rest, partsCounter + 1)
          } else if failOnLimit then {
    ...
    def go(s: Stream[F, Byte], state: Int): Pull[F, Nothing, Stream[F, Byte]] =
      s.pull.uncons.flatMap {
        case Some((chnk, rest)) =>
          ...
          if ix === dashBoundaryBytes.length then Pull.pure(remainder ++ rest)
          else go(rest, ix)
        case None =>
  ...
  private def pullPartChunks[F[_] : Concurrent](
    ...
    def go(
      ...
      if state == delimiterBytes.length then
        Pull.pure(racc ++ s)
      else
        s.pull.uncons.flatMap {
          case Some((chnk, rest)) =>
            val (ix, l, r) = splitOnChunk[F](delimiterBytes, state, chnk, Stream.empty, racc)
            l.chunks.map(PartChunk.apply).pull.echo >> {
              if ix == delimiterBytes.length then Pull.pure(r ++ rest)
              else go(rest, ix, r)
            }
          case None =>
            Pull.raiseError[F](MalformedMessageBodyFailure("Invalid boundary - partial boundary"))
